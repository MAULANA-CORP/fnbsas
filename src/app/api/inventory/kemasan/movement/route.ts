...
/** GET /api/inventory/kemasan/movement — riwayat pergerakan stok Kemasan */
export const GET = withRole(["OWNER", "FINANCE", "PRODUKSI"], async (_user, req) => {
  try {
    ...
    if (itemId) where.kemasanId = itemId;
    ...
    const movements = await prisma.stokMovementKemasan.findMany({
      where,
