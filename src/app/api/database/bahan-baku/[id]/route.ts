...
// PUT /api/database/bahan-baku/:id — hanya OWNER
export const PUT = withOwner<{ params: Promise<{ id: string }> }>(async (user, req, ctx) => {
  try {
    ...
    const existing = await getPrisma().bahanBaku.findUnique({ where: { id } });
    if (!existing) return NextResponse.json({ error: "Data tidak ditemukan" }, { status: 404 });

    const item = await getPrisma().bahanBaku.update({
      where: { id },
      data: {
        nama,
        satuan,
...
// DELETE /api/database/bahan-baku/:id — hanya OWNER
export const DELETE = withOwner<{ params: Promise<{ id: string }> }>(async (user, _req, ctx) => {
  try {
    ...
    await getPrisma().bahanBaku.delete({ where: { id } });
