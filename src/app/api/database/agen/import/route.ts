...
const FIELDS = ENTITY_DEFS.agen.fields;
const WRITE_ROLES = ["OWNER", "SALES"] as const;

// POST /api/database/agen/import — OWNER + SALES
    ...
    for (const row of classified) {
      ...
      try {
        if (row.action === "update" && row.matchedId) {
          await getPrisma().agen.update({ where: { id: row.matchedId }, data });
          summary.updated++;
        } else {
          await getPrisma().agen.create({ data });
          summary.created++;
        }
      } catch {
