...
const FIELDS = ENTITY_DEFS.customers.fields;
const WRITE_ROLES = ["OWNER", "SALES"] as const;

// POST /api/database/customers/import — OWNER + SALES
export const POST = withRole([...WRITE_ROLES], async (user, req) => {
  try {
    ...
    const existing = await getPrisma().customer.findMany({ select: { id: true, nama: true } });
    ...
    for (const row of classified) {
      ...
      const data = {
        ...
        kontak: row.data.kontak ? String(row.data.kontak) : null,
        alamat: row.data.alamat ? String(row.data.alamat) : null,
      };
      try {
        if (row.action === "update" && row.matchedId) {
          await getPrisma().customer.update({ where: { id: row.matchedId }, data });
          summary.updated++;
        } else {
          await getPrisma().customer.create({ data });
          summary.created++;
        }
      } catch {
