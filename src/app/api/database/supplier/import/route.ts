...
const FIELDS = ENTITY_DEFS.supplier.fields;

// POST /api/database/supplier/import — OWNER + FINANCE (lihat catatan di [id]/route.ts)
    ...
    for (const row of classified) {
      ...
      try {
        if (row.action === "update" && row.matchedId) {
          await getPrisma().supplier.update({ where: { id: row.matchedId }, data });
          summary.updated++;
        } else {
          await getPrisma().supplier.create({ data });
          summary.created++;
        }
      } catch {
